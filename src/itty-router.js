function Router({ base = '', routes = [] } = {}) {
  return {
    __proto__: new Proxy(
      {},
      {
        get:
          (target, prop, receiver) =>
          (route, ...handlers) =>
            routes.push([
              prop.toUpperCase(),
              RegExp(
                `^${
                  (base + route)
                    .replace(/(\/?)\*/g, '($1.*)?') // trailing wildcard
                    .replace(/\/$/, '') // remove trailing slash
                    .replace(/:(\w+)(\?)?(\.)?/g, '$2(?<$1>[^/]+)$2$3') // named params
                    .replace(/\.(?=[\w(])/, '\\.') // dot in path
                    .replace(/\)\.\?\(([^\[]+)\[\^/g, '?)\\.?($1(?<=\\.)[^\\.') // optional image format
                }/*$`
              ),
              handlers,
            ]) && receiver,
      }
    ),

    routes,

    /**
     *
     * @param {Request} request - Request
     * @param  {...any} args - Sdditional arguments to pass to the first matched handler
     * @returns {undefined | any | Promise<any>} - A returned promise indicates a route match
     */
    handle(request, ...args) {
      let response,
        match,
        url = new URL(request.url)
      request.query = Object.fromEntries(url.searchParams)
      for (let [method, route, handlers] of routes) {
        if (
          (method === request.method || method === 'ALL') &&
          (match = url.pathname.match(route))
        ) {
          request.params = match.groups
          for (let handler of handlers) {
            if (
              (response = handler(request.proxy || request, ...args)) !==
              undefined
            ) {
              return response
            }
          }
        }
      }
    },
  }
}

module.exports = {
  Router,
}
